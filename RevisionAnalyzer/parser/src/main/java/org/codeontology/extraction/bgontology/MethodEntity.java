...
//    }
    ...
    public void tagSourceCode() {
        getLogger().addTriple(this, BGOntology.SOURCE_CODE_PROPERTY, getModel().createLiteral(getSourceCode()));
    }
    ...
    public void tagAPISeq(){
        if(methodInvocations.size() == 0){
            getInvocations();
        }
        List<String> apiList = new ArrayList<>();
        //调用的非DeclarationAvailable的方法就是API
        for(MethodEntity methodEntity : methodInvocations){
//            if(!methodEntity.isDeclarationAvailable()) {
                apiList.add(methodEntity.getRelativeURI());
//            }
        }
        if(!CollectionUtils.isEmpty(apiList)){
            String apiSeq = String.join(" | ", apiList);
            getLogger().addTriple(this, BGOntology.APISEQ_PROPERTY, getModel().createLiteral(apiSeq));
        }
    }

    public void tagNormalComment(String normalComment){
        CtBlock<?> body = getElement().getBody();
        ...
        if(!StringUtils.isEmpty(normalComment)){
            commentBuilder.append(normalComment + "\n");
        }

        for (CtStatement statement : statements) {
            List<CtComment> comments = statement.getComments();
            commentBuilder.append(parseNormalCommentString(comments));
        }

        //tag
        String commentStr = commentBuilder.toString();
        if(!StringUtils.isEmpty(commentStr)){
            getLogger().addTriple(this, BGOntology.COMMENT_PROPERTY, getModel().createLiteral(commentStr));
        }
    }

    public void tagAllComment(){
        //tag doc comment
        String docComment = getElement().getDocComment();
        if (!StringUtils.isEmpty(docComment)) {
            getLogger().addTriple(this, BGOntology.DOCCOMMENT_PROPERTY, getModel().createLiteral(docComment));
        }
        //方法开头的normal comment
        List<CtComment> comments = new ArrayList<>(getElement().getComments());
        if(comments.size()>=1){
            comments.remove(0);
        }

        //tag normal comment
        String methodNormalComment = parseNormalCommentString(comments);
        tagNormalComment(methodNormalComment);
    }

    private String parseNormalCommentString(List<CtComment> comments){
        if(CollectionUtils.isEmpty(comments)){
            return "";
        }
        StringBuilder commentBuilder = new StringBuilder();
        commentBuilder.append(String.join("\n", comments.stream().map(c->c.getContent()).collect(Collectors.toList())));
