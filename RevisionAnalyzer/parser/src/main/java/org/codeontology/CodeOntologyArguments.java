...
public class CodeOntologyArguments {
    ...
    public static final String IS_ADD_COMMENT_APISEQ = "is-add-comment-apiseq";
...
//        jsap.registerParameter(option);
        ...
        flag = new Switch(IS_ADD_COMMENT_APISEQ);
        flag.setLongFlag(IS_ADD_COMMENT_APISEQ);
        flag.setDefault("false");
        flag.setHelp("this operation is to add comment and api sequence to existed neo4jDB.");
        jsap.registerParameter(flag);
    }
    ...
    public boolean isAddCommentAPISeq(){
        return result.getBoolean(IS_ADD_COMMENT_APISEQ);
    }
}